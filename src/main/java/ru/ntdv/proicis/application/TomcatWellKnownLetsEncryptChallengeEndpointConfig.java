...
public
class TomcatWellKnownLetsEncryptChallengeEndpointConfig
        implements TomcatConnectorCustomizer, ApplicationListener<ApplicationReadyEvent> {
 ...
 *         - SSL properties (serverProperties.ssl) to be used
 * @param domain
 *         - Domain to issue certificate for (i.e. {@code example.com})
 * @param contact
 *         - Your email (i.e. {@code mailto:john.doe@example.com})
 * @param accountKeyAlias
 *         - Key name to store your Account key-pair (necessary for renewal process)
 * @param letsEncryptServer
 *         - LetsEncrypt server to use, as defined in Acme4j. acme://letsencrypt.org is production
 * @param keySize
 *         - RSA Key size for Domain and Account keys that are to be generated
 * @param updateBeforeExpiry
 *         - Start trying to update certificate {@code updateBeforeExpiry} time before it expires
 * @param busyWaitInterval
 *         - How frequently to check if certificate needs update (scheduled-alike busy wait)
 * @param accountCertValidity
 *         - How long account key is valid
 * @param storeCertChain
 *         - Store entire certificate chain or a single certificate
 * @param enabled
 *         - If the helper is enabled (for i.e. development)
 * @param returnNullModel
 *         - If challenge endpoint should return null model, default true
 */
public
...
void customize(Connector connector) {
    ...
    if (!(protocolHandler instanceof final AbstractHttp11Protocol<?> protocol)) {
...
protected
boolean matchesCertFilePathAndPassword(SSLHostConfig config, String password) {
    // Spring Boot 3+
...
private
void createBasicKeystoreIfMissing() {
    File keystoreFile = getKeystoreFile();
    if (keystoreFile.exists()) {
        if (!keystoreFile.canWrite()) {
            throw new IllegalArgumentException(String.format("Keystore %s is not writable, certificate update is impossible",
...
private
TargetProtocol createObservableProtocol(AbstractHttp11Protocol<?> protocol, SSLHostConfig sslConfig) {
    ...
    if (null == cert) {
        logger.warn(
                ...
                protocol.getPort(),
                sslConfig.getCertificates().stream().map(SSLHostConfigCertificate::getCertificateKeystoreFile)
...
private
void saveKeystore(File keystoreFile, KeyStore keystore) {
    try (var os = Files.newOutputStream(keystoreFile.toPath(), StandardOpenOption.CREATE, StandardOpenOption.WRITE,
...
private
void letsEncryptCheckCertValidityAndRotateIfNeeded() {
    while (true) {
        try {
...
private
void executeCheckCertValidityAndRotateIfNeeded() {
    var ks = tryToReadKeystore();
    for (var protocol : observedProtocols) {
        ...
        if (null == cert) {
            logger.warn("Certificate is null on {}:{} from {}",
                        ...
                        protocol.getHostConfig().getCertificates().stream()
                                .map(SSLHostConfigCertificate::getCertificateKeystoreFile).collect(Collectors.toList())
                       );
            continue;
        }
...
private
KeyStore createBasicKeystoreWithSelfSignedCert() {
    try {
        ...
        newKeystore.setKeyEntry(serverProperties.getSsl().getKeyAlias(), domainKey.getPrivate(), keyPassword().toCharArray(),
                                new Certificate[] { signedDomain });
        newKeystore.setKeyEntry(accountKeyAlias, accountKey.getPrivate(), keyPassword().toCharArray(),
                                new Certificate[] { signedAccount });
        return newKeystore;
...
private
Certificate selfSign(KeyPair keyPair, Instant notBefore, Instant notAfter) {
    ...
    try {
        var contentSigner = new JcaContentSignerBuilder("SHA256WithRSA").setProvider(BouncyCastleProvider.PROVIDER_NAME)
...
private
void updateCertificateAndKeystore(KeyStore ks) {
    ...
    try {
        ...
        var accountKeyEntry = (KeyStore.PrivateKeyEntry) ks.getEntry(accountKeyAlias, new KeyStore.PasswordProtection(
                keyPassword().toCharArray()));
        var domainKeyEntry = (KeyStore.PrivateKeyEntry) ks.getEntry(serverProperties.getSsl().getKeyAlias(),
        ...
        ks.setKeyEntry(
                ...
                keyPassword().toCharArray(),
                storeCertChain ? certificate.getCertificateChain().toArray(new X509Certificate[0])
                               : new Certificate[] { certificate.getCertificate() }
                      );
...
private
org.shredzone.acme4j.Certificate authorizeOrder(KeyPair domainKey, Order order) throws AcmeException, IOException {
    try {
...
private
void finalizeOrder(Order order, byte[] csrb) throws AcmeException {
    try {
        order.execute(csrb);
...
private
void waitForAuthStatus(Authorization auth, Set<Status> authNotInStatus) throws AcmeException {
...
private
void waitForOrderStatus(Order order, Set<Status> orderNotInStatus) throws AcmeException {
    // Wait for the order to complete
    try {
        int attempts = 10;
...
private
X509Certificate tryToReadCertificate(TargetProtocol protocol, KeyStore ks) {
    if (null == ks) {
...
private
String readCertificateAliasFromProtocol(TargetProtocol protocol) {
...
private
String parseCertificateKeystoreFilePath(String path) {
    return path.replaceAll("file://", "").replaceAll("file:", "");
}

private
String keyPassword() {
    return null == serverProperties.getSsl().getKeyPassword() ? serverProperties.getSsl().getKeyStorePassword()
                                                              : serverProperties.getSsl().getKeyPassword();
}

private
void dumpOrderStatusOnError(Order order) {
    logger.warn("Order action failed, status is: {}, see actual order status here: {}", order.getStatus(),
                order.getLocation());
    if (null == order.getAuthorizations()) {
        return;
    }
    for (var auth : order.getAuthorizations()) {
        logger.warn("Order {} authorization status is: {}, actual status here: {}", order.getIdentifiers(), auth.getStatus(),
                    auth.getLocation());
    }
}

public static
class WellKnownLetsEncryptChallenge extends AbstractController {

    private final Map<String, String> challengeToken;
    private final boolean returnNullModel;

    public
    WellKnownLetsEncryptChallenge(Map<String, String> challengeToken, boolean returnNullModel) {
...
class TargetProtocol {
    ...
    public
    TargetProtocol(SSLHostConfig hostConfig, AbstractHttp11Protocol protocol) {
        this.hostConfig = hostConfig;
        this.protocol = protocol;
    }

    public
    SSLHostConfig getHostConfig() {
        return hostConfig;
    }

    public
    AbstractHttp11Protocol getProtocol() {
        return protocol;
    }
}
}
